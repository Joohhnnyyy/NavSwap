...
@keyframes scale {
  ...
  if (variant === "circle-blur") {
    const getClipPathAt = (position: AnimationStart) => {
      switch (position) {
    ...
    const atPos = getClipPathAt(start);
    ...
    return {
      ...
      ::view-transition-new(root) {
        animation: reveal-circle-blur 1s;
      ...
      @keyframes reveal-circle-blur {
        from {
          clip-path: circle(0% at ${atPos});
        }
        to {
          clip-path: circle(150% at ${atPos});
        }
      }
      `,
    };
  ...
  return {
    ...
    css: `
      ...
      @keyframes scale-${start}${blur ? "-blur" : ""} {
        from {
          /* blur animation removed for performance */
        }
        to {
          mask-size: 2000vmax;
        }
