...
import { LogEntry } from './map/types';
...
const EventLog: React.FC<EventLogProps> = ({ logs, theme, isSimulating, className }) => {
  ...
  return (
    <div className={cn("absolute bottom-[260px] right-12 z-[5000] w-72 h-[300px] flex flex-col pointer-events-none", className)}>
      <div className={`
        frosted-glass rounded-[3rem] overflow-hidden flex flex-col h-full transition-all duration-700 pointer-events-auto
        ${isTactical ? 'text-white bg-[#1c1c1e]/80' : 'text-slate-900 border-white shadow-2xl bg-white/80'}
      `}>
